 ...
 * 
 * Author: Andre Charles Legendre, based on code from Luke Hutchison (luke.hutch@mit.edu)
 *   Please drop me an email if you use this code so I can list your project here!
 * 
 * Usage:
 *   <code>
 *   public class MyMTView extends View implements MultiTouchImageController<PinchWidgetType> {
 *
 ...
 *       }
 *     
 *       // ... then implement the MultiTouchImageController interface here, see details in the comments of that interface.
 *   }
 *   </code>
 * 
 * License:
 ...
 */
public class MultiTouchController<ImageController> {
	...
	MultiTouchImageController<ImageController> multitouchImageController;
	...
	public MultiTouchController(MultiTouchImageController<ImageController> objectCanvas) {
		this(objectCanvas, true);
	}

	/** Full constructor */
	public MultiTouchController(MultiTouchImageController<ImageController> objectCanvas, boolean handleSingleTouchEvents) {
		this.mCurrPt = new PointInfo();
		this.mPrevPt = new PointInfo();
		this.handleSingleTouchEvents = handleSingleTouchEvents;
		this.multitouchImageController = objectCanvas;
	}
	...
	private void anchorAtThisPositionAndScale() {
		Log.i("MultiTouchController", "anchorAtThisPositionAndScale selectedObject " + selectedObject);
		if (selectedObject == -1)
			return;

		// Get selected object's current position and scale
		multitouchImageController.getPositionAndScale(selectedObject, mCurrXform);
	...
	private boolean anchorAtThisPositionAndScale(String args) {
		String[] params = cutString(args, ",");
		if (params.length >= 3) {
			this.selectedObject = Integer.parseInt(params[0]);
			this.mCurrPtX = Float.parseFloat(params[1]);
			this.mCurrPtY = Float.parseFloat(params[2]);
			this.mCurrPtDiam = Float.parseFloat(params[3]);
			this.mCurrPtWidth = Float.parseFloat(params[4]);
			this.mCurrPtHeight = Float.parseFloat(params[5]);
			this.mCurrPtAng = Float.parseFloat(params[6]);
	...
	private void anchorAtThisPositionAndScale(int selectedObject, float mCurrPtX, float mCurrPtY, float mCurrPtDiam, float mCurrPtWidth, float mCurrPtHeight, float mCurrPtAng) {
		this.selectedObject = selectedObject;
		this.mCurrPtX = mCurrPtX;
		this.mCurrPtY = mCurrPtY;
		this.mCurrPtDiam = mCurrPtDiam;
		this.mCurrPtWidth = mCurrPtWidth;
		this.mCurrPtHeight = mCurrPtHeight;
		this.mCurrPtAng = mCurrPtAng;
		anchorAtThisPositionAndScale();
	}
	...
	private void performDragOrPinch() {
		...
		Log.i("MultiTouchController", "performDragOrPinch selectedObject " + selectedObject);
		if (selectedObject == -1)
			return;
		...
		boolean success = multitouchImageController.setPositionAndScale(selectedObject, mCurrXform, mCurrPt);
		if (!success)
	...
	private void multiTouchController() {
		...
		case MODE_NOTHING:
			...
			if (mCurrPt.isDown()) {
				...
				selectedObject = multitouchImageController.getDraggableObjectAtPoint(mCurrPt);
				if (selectedObject != -1) {
					// Started a new single-point drag
					mMode = MODE_DRAG;
					multitouchImageController.selectObject(selectedObject, mCurrPt);
					anchorAtThisPositionAndScale();
...
Log.e("NOMBRE", "MODE_PINCH sendRemotes " + "ACTION_MOVE," + selectedObject + "," + mCurrXform.toString());
	...
	public static class PositionAndScale {
		...
		public boolean fromString(String args) {
			String[] params = cutString(args, ",");
			if (params.length >= 3) {
				this.xOff = Float.parseFloat(params[0]);
				this.yOff = Float.parseFloat(params[1]);
				this.scale = Float.parseFloat(params[2]);
				this.scaleX = Float.parseFloat(params[3]);
				this.scaleY = Float.parseFloat(params[4]);
				this.angle = Float.parseFloat(params[5]);
				return true;
			} else {
				return false;
			}
		...
		public String toString() {
			return xOff + "," + yOff + "," + scale + "," + scaleX + "," + scaleY + "," + angle;
		}
	...
	public static interface MultiTouchImageController<T> {
		 ...
		 */
		public int getDraggableObjectAtPoint(PointInfo touchPoint);
		 ...
		 */
		public void getPositionAndScale(int selectedObject, PositionAndScale objPosAndScaleOut);
		 ...
		 *         for this object.
		 */
		public boolean setPositionAndScale(int selectedObject, PositionAndScale newObjPosAndScale, PointInfo touchPoint);
		 ...
		 *            The current touch point.
		 */
		public void selectObject(int selectedObject, PointInfo touchPoint);
